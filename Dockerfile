...
# Pass in DIJETH_COMMIT as an arg to allow the build script to set this externally
ARG DIJETH_COMMIT
ARG CURRENT_BRANCH

RUN export DIJETH_COMMIT=$DIJETH_COMMIT && export CURRENT_BRANCH=$CURRENT_BRANCH && ./scripts/build.sh /build/evm
